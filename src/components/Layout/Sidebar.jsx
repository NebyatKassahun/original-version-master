...
import { LayoutDashboard, Package, Warehouse, ShoppingCart, TrendingUp, ShoppingBag, Users, BarChart3, Settings, LogOut, Menu, Layers } from 'lucide-react';
            ...
            return (
              <li key={item.path}>
                <div className="relative">
                  <NavLink
                    to={item.path}
                    className={`flex items-center px-3 py-2.5 rounded-xl transition-all duration-200 group relative mb-1 ${active
                      ? 'bg-gradient-to-r from-blue-600 to-blue-700 text-white shadow-lg transform scale-[1.02]'
                      : 'text-slate-300 hover:bg-slate-700/50 hover:text-white hover:transform hover:scale-[1.01]'
                      }`}
                  >
                    <Icon className={`w-5 h-5 ${state.sidebarCollapsed ? 'mx-auto' : 'mr-3'} transition-transform duration-200 ${active ? 'scale-110' : ''}`} />
